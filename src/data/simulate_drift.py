...
def main():
    ...
    if not ref.exists():
        raise FileNotFoundError(
            "Reference not found. Run: python -m src.features.transform"
        )
    ...
    if "trip_distance" in drift:
        drift["trip_distance"] = (
            drift["trip_distance"] * rng.normal(1.2, 0.1, len(drift))
        ).clip(lower=0)
    if "hour" in drift:
    ...
    if "duration_min" in drift:
        drift["duration_min"] = (
            drift["duration_min"] * rng.normal(1.1, 0.15, len(drift))
        ).clip(1, 180)
